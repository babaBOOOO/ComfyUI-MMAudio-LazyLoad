...
class LazyMMAudioModel:
    def __init__(self, model_path, model_name, base_precision):
    ...
    def _load_model(self):
        ...
        base_dtype = {"fp8_e4m3fn": torch.float8_e4m3fn, "fp8_e4m3fn_fast": torch.float8_e4m3fn, "bf16": torch.bfloat16, "fp16": torch.float16, "fp32": torch.float32}[self.base_precision]
        
        mmaudio_sd = load_torch_file(self.model_path, device=offload_device)
        ...
        del mmaudio_sd
        log.info(f'Loaded MMAudio model weights from {self.model_path}')
        if "44" in self.model_name:
            model.seq_cfg = CONFIG_44K
        elif "16" in self.model_name:
    ...
    def __getattr__(self, name):
        # Forward all attribute access to the actual model
        if name in ['model_path', 'model_name', 'base_precision', '_model', '_loaded']:
            return object.__getattribute__(self, name)
        return getattr(self._load_model(), name)
    
    def to(self, device):
        return self._load_model().to(device)
...
class MMAudioModelLoader:
    ...
    def loadmodel(self, mmaudio_model, base_precision):
        # Return lazy wrapper instead of loading the model immediately
        mmaudio_model_path = folder_paths.get_full_path_or_raise("mmaudio", mmaudio_model)
        return (LazyMMAudioModel(mmaudio_model_path, mmaudio_model, base_precision),)
    
#region Features Utils
class LazyMMAudioVoCoder:
    def __init__(self, vocoder_path, vocoder_name):
    ...
    def _load_vocoder(self):
        ...
        vocoder = BigVGAN.from_pretrained(self.vocoder_path).eval()
        self._vocoder = vocoder
        self._loaded = True
        return vocoder
    
    def __getattr__(self, name):
        # Forward all attribute access to the actual vocoder
        if name in ['vocoder_path', 'vocoder_name', '_vocoder', '_loaded']:
            return object.__getattribute__(self, name)
        return getattr(self._load_vocoder(), name)
...
class MMAudioVoCoderLoader:
    ...
    def loadmodel(self, vocoder_model):
        ...
        return (LazyMMAudioVoCoder(vocoder_model_path, vocoder_model),)
        
class LazyMMAudioFeatureUtils:
    def __init__(self, vae_path, vae_name, synchformer_path, synchformer_name, clip_path, clip_name, bigvgan_vocoder_model, mode, precision):
        self.vae_path = vae_path
        self.vae_name = vae_name
        self.synchformer_path = synchformer_path
        self.synchformer_name = synchformer_name
        self.clip_path = clip_path
        self.clip_name = clip_name
        self.bigvgan_vocoder_model = bigvgan_vocoder_model
        self.mode = mode
        self.precision = precision
        self._feature_utils = None
        self._loaded = False
    
    def _load_feature_utils(self):
        if self._loaded:
            return self._feature_utils
        ...
        # Load synchformer
        synchformer_sd = load_torch_file(self.synchformer_path, device=offload_device)
        with init_empty_weights():
        ...
        if self.mode == "44k":
            ...
            # For 16k mode, use the provided vocoder model
            assert self.bigvgan_vocoder_model is not None, "bigvgan_vocoder_model must be provided for 16k mode"
            bigvgan_vocoder = self.bigvgan_vocoder_model
        
        vae_sd = load_torch_file(self.vae_path, device=offload_device)
        ...
        clip_sd = load_torch_file(self.clip_path, device=offload_device)
    ...
    def __getattr__(self, name):
        # Forward all attribute access to the actual feature_utils
        if name in ['vae_path', 'vae_name', 'synchformer_path', 'synchformer_name', 'clip_path', 'clip_name', 'bigvgan_vocoder_model', 'mode', 'precision', '_feature_utils', '_loaded']:
            return object.__getattribute__(self, name)
        return getattr(self._load_feature_utils(), name)
    
    def to(self, device):
        return self._load_feature_utils().to(device)
...
class MMAudioFeatureUtilsLoader:
    ...
    def loadmodel(self, vae_model, precision, synchformer_model, clip_model, mode, bigvgan_vocoder_model=None):
        # Return lazy wrapper instead of loading models immediately
        ...
        return (LazyMMAudioFeatureUtils(vae_path, vae_model, synchformer_path, synchformer_model, clip_model_path, clip_model, bigvgan_vocoder_model, mode, precision),)
...
class MMAudioSampler:
    ...
    def sample(self, mmaudio_model, seed, feature_utils, duration, steps, cfg, prompt, negative_prompt, mask_away_clip, force_offload, images=None):
        ...
        # Models will be loaded on-demand when accessed
        log.info("MMAudio models will be loaded on-demand when needed...")
